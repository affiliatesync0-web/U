      ...
      <div className="space-y-6 md:space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-sm md:text-base text-slate-500 font-medium">Registro completo y transparente de cada transferencia bancaria recibida.</p>
          </div>
                ...
                <Card key={sale.id} className="border-none shadow-sm rounded-2xl overflow-hidden bg-white border border-slate-50">
                  <CardContent className="p-6 space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] font-mono font-bold text-muted-foreground bg-slate-100 px-2 py-1 rounded-md">REF: {sale.voucherReference || 'N/A'}</span>
                        <h3 className="font-bold text-base mt-2 text-slate-800">{sale.productName || sale.productId}</h3>
                        <Badge variant="outline" className="text-purple-600 border-purple-100 bg-purple-50 text-[9px] font-black uppercase tracking-widest mt-1"><Landmark className="h-3 w-3 mr-1" /> {t.bankTransfer}</Badge>
                      </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
                        <TableHead className="px-8 h-14 uppercase text-[10px] font-bold tracking-widest">Referencia / Pago</TableHead>
                        ...
                        <TableRow key={sale.id} className="hover:bg-slate-50/50 transition-colors h-16">
                          <TableCell className="px-8">
                            <div className="flex flex-col gap-1">
                              <span className="font-mono font-black text-xs text-primary">{sale.voucherReference || 'SIN REF'}</span>
                              <Badge variant="outline" className="w-fit text-purple-600 border-purple-100 bg-purple-50 text-[8px] font-black uppercase tracking-widest"><Landmark className="h-2 w-2 mr-1" /> {t.bankTransfer}</Badge>
                            </div>
                          </TableCell>
