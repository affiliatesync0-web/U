...
import { ShoppingBag, Target, Loader2, Smartphone, ShieldCheck, UserCheck } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4">
      ...
      <div className="w-full max-w-md text-center mb-10">
        <h1 className="text-4xl font-headline font-black text-slate-900 tracking-tight">Crea tu Cuenta</h1>
        <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-2">Únete a la red líder en Nicaragua</p>
      </div>
      ...
      {step === 'info' && (
        <Card className="w-full max-w-xl border-none shadow-2xl rounded-[3rem] p-8 md:p-12 animate-in slide-in-from-bottom-4 duration-500">
          <div className="space-y-8">
            {!auth.currentUser && (
              <>
                <Button variant="outline" onClick={handleGoogleRegister} disabled={loading} className="w-full h-14 rounded-2xl font-bold gap-3 border-slate-200">
                ...
                <div className="relative"><div className="absolute inset-0 flex items-center"><span className="w-full border-t" /></div><div className="relative flex justify-center text-[10px] font-black uppercase text-slate-400"><span className="bg-white px-2">O completa tus datos</span></div></div>
              </>
            )}
            ...
            <form onSubmit={role === 'affiliate' ? (e) => { e.preventDefault(); setStep('exam'); } : handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder="Ej: Juan" value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} required className="h-12 rounded-xl" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Apellido</Label>
                  <Input placeholder="Ej: Perez" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} required className="h-12 rounded-xl" />
                </div>
              </div>
              ...
              <Button type="submit" className="w-full h-16 rounded-2xl font-black text-lg shadow-xl shadow-primary/20 transition-all active:scale-95" disabled={loading}>
                {loading ? <Loader2 className="animate-spin h-6 w-6" /> : (role === 'affiliate' ? "Siguiente: Evaluación" : "Completar Registro")}
              </Button>
      ...
      {step === 'exam' && (
        <Card className="w-full max-w-xl border-none shadow-2xl rounded-[3rem] p-10 animate-in slide-in-from-right-4 duration-500">
          <CardHeader className="p-0 mb-8">
            <CardTitle className="text-2xl font-headline font-black text-primary flex items-center gap-3">
              <UserCheck className="h-6 w-6" /> Evaluación de Afiliado
            </CardTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button type="submit" className="w-full h-16 rounded-2xl font-black text-lg shadow-xl shadow-primary/20" disabled={loading}>
              {loading ? <Loader2 className="animate-spin h-6 w-6" /> : "Finalizar y enviar"}
            </Button>
