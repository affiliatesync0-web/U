      ...
      {step === 'role' && (
        <div className="w-full max-w-4xl space-y-10 animate-in fade-in zoom-in-95 duration-500">
          <div className="text-center">
            <h1 className="text-5xl font-headline font-black text-slate-900 tracking-tight leading-none">Crea tu Cuenta</h1>
          ...
          <div className="space-y-10">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <h2 className="text-3xl font-headline font-black text-slate-900 tracking-tight leading-none">Tus Datos</h2>
            ...
            <form onSubmit={role === 'affiliate' ? (e) => { e.preventDefault(); setStep('exam'); } : handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} required className="h-14 rounded-2xl bg-slate-50 border-none ring-1 ring-slate-100 px-5 font-bold" placeholder="Juan" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Apellido</Label>
                  <Input value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} required className="h-14 rounded-2xl bg-slate-50 border-none ring-1 ring-slate-100 px-5 font-bold" placeholder="Pérez" />
                </div>
              </div>
      ...
      {step === 'exam' && (
        <Card className="w-full max-w-xl border-none shadow-2xl rounded-[3.5rem] p-10 md:p-14 animate-in slide-in-from-right-8 duration-500">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-2"><Label className="text-[11px] font-bold text-slate-600">¿Cómo planeas promocionar nuestros productos?</Label><Textarea required value={examData.q1} onChange={e => setExamData({...examData, q1: e.target.value})} className="rounded-2xl min-h-[100px] bg-slate-50 border-none ring-1 ring-slate-100 p-5 text-sm font-medium" placeholder="TikTok, Facebook Ads, WhatsApp Marketing..." /></div>
            <div className="space-y-2"><Label className="text-[11px] font-bold text-slate-600">¿Cuál es tu experiencia en el mundo digital?</Label><Textarea required value={examData.q2} onChange={e => setExamData({...examData, q2: e.target.value})} className="rounded-2xl min-h-[100px] bg-slate-50 border-none ring-1 ring-slate-100 p-5 text-sm font-medium" placeholder="Cuéntanos un poco sobre ti..." /></div>
            ...
            <div className="p-6 bg-blue-50 border border-blue-100 rounded-[2rem] flex items-start gap-4">
               ...
               <p className="text-[10px] font-black text-blue-700 leading-relaxed uppercase tracking-widest">
                 Tu solicitud será revisada por nuestro equipo. Recibirás un correo cuando tu cuenta sea habilitada.
               </p>
            </div>
