...
/**
 * Página de Acceso Administrativo Reforzada.
 * Incluye diagnóstico automático de dominios para entornos de desarrollo.
 */
  ...
  const handleGoogleAdminLogin = async () => {
    ...
    try {
      ...
      if (user?.email?.toLowerCase() === allowedEmail.toLowerCase()) {
        ...
        toast({ variant: "destructive", title: "Email Incorrecto", description: "Esta cuenta no es la de administración." });
      }
    } catch (error: any) {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        errorMessage = "La ventana se cerró antes de terminar.";
          ...
          <CardContent className="p-0 space-y-6">
            ...
            <Alert className="bg-blue-50 border-blue-100 rounded-2xl border-2">
              ...
              <AlertDescription className="text-xs text-blue-700 font-bold">
                Email oficial: <span className="text-blue-900 block mt-1">affiliatesync0@gmail.com</span>
              </AlertDescription>
            </Alert>
            ...
            {(authErrorCode === 'auth/unauthorized-domain' || authErrorCode === 'auth/network-request-failed') && (
              <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                <div className="p-4 bg-white rounded-2xl border-2 border-red-100 shadow-inner flex flex-col gap-3">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Tu Dominio Actual:</p>
            ...
            {wrongEmail && (
              <Alert variant="destructive" className="rounded-2xl border-2 bg-amber-50 border-amber-100">
                ...
                <AlertTitle className="text-xs font-black uppercase text-amber-900">Email Incorrecto</AlertTitle>
                <AlertDescription className="text-[10px] mt-1 font-bold text-amber-800">
                  Entraste con: <b className="underline">{wrongEmail}</b>. Usa el botón de abajo para elegir la cuenta correcta.
                </AlertDescription>
              </Alert>
            )}
