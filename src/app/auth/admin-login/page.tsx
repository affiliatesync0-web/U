...
import { ShieldAlert, ArrowLeft, Loader2, AlertCircle, ExternalLink, ShieldCheck, Globe, UserX, Copy, Check, RefreshCw } from 'lucide-react'
...
/**
 * Página de Acceso Administrativo Blindada.
 * Diseñada para diagnosticar y resolver bloqueos de dominios en entornos de desarrollo.
 */
export default function AdminLoginPage() {
  ...
  const handleGoogleAdminLogin = async () => {
    ...
    try {
      // 1. Forzar cierre de sesión previo para evitar conflictos de cuenta de afiliado
      await signOut(auth);
      
      const provider = new GoogleAuthProvider();
      // Forzar a Google a mostrar siempre el selector de cuentas
      ...
      if (user?.email?.toLowerCase() === allowedEmail.toLowerCase()) {
        ...
        await signOut(auth); // Desloguear inmediatamente si no es el admin
        setWrongEmail(attemptedEmail || "desconocido");
        toast({ 
          variant: "destructive", 
          title: "Email Incorrecto", 
          description: `La cuenta ${attemptedEmail} no tiene permisos de administración.` 
        });
      }
    } catch (error: any) {
      ...
      let errorMessage = "Ocurrió un error al intentar conectar con Google.";
      
      if (error.code === 'auth/popup-closed-by-user') {
        errorMessage = "La ventana se cerró antes de elegir cuenta. Revisa si tienes bloqueadores de pop-ups.";
      } else if (error.code === 'auth/unauthorized-domain') {
        errorMessage = "Este dominio no está autorizado en Firebase. Sigue las instrucciones abajo.";
      } else if (error.code === 'auth/network-request-failed') {
        errorMessage = "Error de red o conexión bloqueada por el navegador.";
      }
        ...
        <div className="bg-slate-50/50 rounded-[3rem] p-10 md:p-12">
          <CardHeader className="text-center space-y-4 p-0 mb-10">
            ...
            <div className="space-y-1">
              <CardTitle className="text-3xl font-headline font-black text-slate-900 tracking-tight">Acceso Admin</CardTitle>
              <CardDescription className="font-bold text-[10px] uppercase tracking-widest text-slate-400">Solo Personal Autorizado</CardDescription>
            </div>
          </CardHeader>

          <CardContent className="p-0 space-y-6">
            {/* Alerta de Correo Autorizado */}
            <Alert className="bg-blue-50 border-blue-100 rounded-2xl border-2">
              ...
              <AlertDescription className="text-xs text-blue-700 font-bold">
                Email administrativo: <span className="text-blue-900 block mt-1">affiliatesync0@gmail.com</span>
              </AlertDescription>
            </Alert>

            {/* DIAGNÓSTICO CRÍTICO: Se muestra si hay error o si el usuario no puede entrar */}
            {(authErrorCode || wrongEmail) && (
              <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
                <Alert variant="destructive" className="rounded-2xl border-2 bg-red-50">
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle className="text-xs font-black uppercase">Bloqueo Detectado</AlertTitle>
                  <AlertDescription className="text-[10px] mt-2 font-bold text-red-900 leading-relaxed">
                    Firebase bloquea el acceso si no autorizas este dominio específico en tu consola:
                  </AlertDescription>
                </Alert>
                
                <div className="p-4 bg-white rounded-2xl border-2 border-red-100 shadow-inner flex flex-col gap-3">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Copia este Dominio:</p>
                  ...
                  <a 
                    ...
                    target="_blank" 
                    className="w-full h-12 bg-red-600 hover:bg-red-700 text-white rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-red-200"
                  >
            ...
            {wrongEmail && (
              <Alert variant="destructive" className="rounded-2xl border-2 bg-amber-50 border-amber-100">
                ...
                <AlertDescription className="text-[10px] mt-1 font-bold text-amber-800">
                  Intentaste con: <b className="underline">{wrongEmail}</b>. Debes elegir la cuenta administrativa autorizada.
                </AlertDescription>
              </Alert>
            )}
            ...
            <div className="pt-6 border-t border-slate-100 flex flex-col items-center gap-2 text-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Sincronización Sync Connect Activa</span>
              </div>
            </div>
          </CardContent>
        </div>
